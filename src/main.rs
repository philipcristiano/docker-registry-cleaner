...
async fn main() -> anyhow::Result<()> {
    ...
    let client = Client::configure().registry(&args.registry).build()?;
    let layer_type = docker_registry::mediatypes::MediaTypes::ManifestV2S1;
    let s1s_client = Client::configure().registry(&args.registry).accepted_types(Some(vec!((layer_type, None)
    ...
    let catalog_stream  = client.get_catalog(None);

    futures::pin_mut!(catalog_stream);
    while let Ok(Some(repo)) = catalog_stream.try_next().await {
        println!("Repo {repo:?}");
        let tag_stream = client.get_tags(&repo, None);
        futures::pin_mut!(tag_stream);
        while let Ok(Some(tag)) = tag_stream.try_next().await{
            let image = format!("{repo}:{tag}");
            println!("{image}");
            let manifest = client.get_manifest(&repo, &tag).await;
            //println!("manifest {manifest:?}");
            match manifest {
                Err(_e) => {},
                Ok(Manifest::S1Signed(signed_manifest)) => {
                    let l = signed_manifest.get_labels(0);
                    println!("Labels {l:?}")

                }
                Ok(Manifest::ML(l)) => {

                    for manifest_obj in l.manifests {
                        let d = manifest_obj.digest;
                        println!("digest {d:?}");
                        let digest_manifest = client.get_manifest(&repo, &d).await;

                        println!("digest manifest {digest_manifest:?}");
                        match digest_manifest {
                            Err(_e) => {},
                            Ok(Manifest::S1Signed(signed_manifest)) => {
                                let l = signed_manifest.get_labels(0);
                                println!("Labels {l:?}")
                            ...
                            Ok(Manifest::S2(s2)) => {
                                let s2_digest = &s2.manifest_spec.config().digest;
                                println!("S2 digest {s2_digest:?}");
                                let b = client.get_blob_response(&repo, &s2_digest).await;
                                println!("digest blob {b:?}");
